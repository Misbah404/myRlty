...
const getTime = (time) => {
  return new Date('2022-08-03T00:00:00.000Z'.replace('00:00:00',  time)).toLocaleString('en-US', {hour: 'numeric', minute: 'numeric', hour12: true});
};
module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    try {
      const findOptions = await sails.helpers.getOptions.with({ searchFrom: ['agency_name','full_name'], ...inputs });
      ...
      for (let i = 0; i < userLogic.length; i++) {
        // userLogic[i].availability_from = getTime(userLogic[i].availability_from);
        // userLogic[i].availability_to = getTime(userLogic[i].availability_to);
        const u = userLogic[i];
